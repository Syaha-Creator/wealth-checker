...
describe("generatePdfReport", () => {
  it("berhasil generate PDF untuk data lengkap (rentang 1 bulan)", async () => {
    const bytes = await generatePdfReport(baseReportData());
    expect(bytes.length).toBeGreaterThan(0);
    // Header standar file PDF
    expect(Buffer.from(bytes.slice(0, 5)).toString("ascii")).toBe("%PDF-");
  });

  it("berhasil generate PDF untuk data KOSONG (user baru, belum ada transaksi)", async () => {
    ...
    const bytes = await generatePdfReport(empty);
    expect(bytes.length).toBeGreaterThan(0);
  });

  it("berhasil generate PDF untuk data BANYAK (>100 transaksi, rentang 12 bulan)", async () => {
    const many = baseReportData({
      from: "2026-01-01",
      to: "2026-12-31",
      transactionListWithinPdfLimit: false, // rentang > 3 bulan, daftar transaksi tidak dirender di PDF
      monthlyPL: Array.from({ length: 12 }, (_, i) => ({
        bulan: `2026-${String(i + 1).padStart(2, "0")}`,
        pendapatan: 10_000_000,
        pinjamanMasuk: 0,
        bayarUtang: 0,
        piutangTerbayar: 0,
        pengeluaran: 6_000_000,
        tabungan: 4_000_000,
        tabunganNegatif: false,
      })),
      transactionList: Array.from({ length: 150 }, (_, i) => ({
        tanggal: "2026-01-01",
        type: "pengeluaran",
        kategori: "Konsumsi",
    ...
    const bytes = await generatePdfReport(many);
    expect(bytes.length).toBeGreaterThan(0);
  });

  it("rentang 12 bulan dengan transactionListWithinPdfLimit=true tetap tidak error (>100 baris tabel, banyak halaman)", async () => {
    const many = baseReportData({
      transactionListWithinPdfLimit: true,
      transactionList: Array.from({ length: 200 }, (_, i) => ({
        tanggal: "2026-01-01",
        type: "pengeluaran",
        kategori: "Konsumsi",
        rincian: `Transaksi dengan rincian yang cukup panjang ke-${i}`,
        nominal: 50_000,
      })),
    });
    const bytes = await generatePdfReport(many);
