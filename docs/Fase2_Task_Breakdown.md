...
- [x] **Medium #14** (ditemukan saat audit bug-hunter round 2): `.finite()` guard (Medium #8) ternyata terlewat di `profile.ts` — `pemasukanBulananRataRata`/`pengeluaranBulananRataRata` masih `nonnegative()` tanpa `.finite()`, jadi `Infinity` lolos Zod lalu gagal 500 generik saat di-cast ke kolom `numeric`. Ditambahkan `.finite()` konsisten dengan pola di accounts/assets/debts/transactions.
- [x] **Medium #15**: `GET /api/budgeting-advice` di frontend (`budgeting/page.tsx`) tidak mengirim `bulanTahun` eksplisit, jadi bergantung pada default `currentYm()` di server (timezone server, bisa UTC) — sedangkan `POST /budget-plans` sudah mengirim `bulanTahun` yang dihitung di browser (timezone lokal user/WIB). Di sekitar tengah malam WIB (±7 jam/bulan karena selisih UTC+7), GET dan POST bisa merujuk bulan yang berbeda sehingga rencana yang baru disimpan tidak langsung muncul. Fix: kedua `fetch()` GET sekarang juga mengirim `?bulanTahun=${currentYm()}` yang dihitung di client, konsisten dengan POST.
- [x] Deploy Fase 2 ke production — sudah di-push ke `main` (deploy otomatis terjadi lewat CI, lihat `.github/workflows/deploy.yml`)
